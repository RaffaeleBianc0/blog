...
// This version supports multiple ToC instances (e.g., desktop and mobile versions).
...
const tocSelector = ".widget--toc";  // Changed from ID selector to class selector to support multiple instances
const navigationQuery = ".widget--toc #TableOfContents li";
...
function setupScrollspyForToC(tocContainer: HTMLElement) {
    ...
    let scrollableNavigation = tocContainer.querySelector("#TableOfContents") as HTMLElement | undefined;
    if (!scrollableNavigation) {
        console.warn("No #TableOfContents found in", tocContainer);
        return;
    }

    let navigation = tocContainer.querySelectorAll("#TableOfContents li");
    if (!navigation || navigation.length === 0) {
        console.warn("No navigation items found in ToC");
        return;
    }
...
export function setupScrollspy() {
    // Find all ToC containers and setup scrollspy for each one
    const tocContainers = document.querySelectorAll(tocSelector);
    
    if (!tocContainers || tocContainers.length === 0) {
        console.warn("No ToC containers found with selector", tocSelector);
        return;
    }

    tocContainers.forEach((tocContainer) => {
        setupScrollspyForToC(tocContainer as HTMLElement);
    });
}
